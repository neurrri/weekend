...
class WordTableViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        var rowCount = wordData.allWords.count
        return rowCount
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let wordSet = wordData.allWords[indexPath.row]
    ...
    @IBAction func saveNewWordSetButton(_ sender: Any) {
        if let newKeyword = self.newKeyword.text, let newExplanation = self.newExplanation.text {
            if newKeyword == "" || newExplanation == "" {
                ...
                wordData.allWords.insert(Words(keyword: newKeyword, explanation: newExplanation), at: 0)
            }
        }
